...
public class IceTridentEntity extends TridentEntity {
    ...
    public IceTridentEntity(EntityType<? extends IceTridentEntity> entityType, World world) {
        super(entityType, world);
        this.tridentStack = new ItemStack(ModItems.ICE_TRIDENT);
    }

    public IceTridentEntity(World world, LivingEntity owner, ItemStack stack) {
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                for (int k = -1; k <= 1; k++) {
                    if (this.world.getBlockState(pos.add(i, j, k)) == Blocks.WATER.getDefaultState()) {
                        if (this.random.nextBoolean())
                            this.world.setBlockState(pos.add(i, j, k), Blocks.ICE.getDefaultState());
                        else
                            this.world.setBlockState(pos.add(i, j, k), Blocks.PACKED_ICE.getDefaultState());
                    }
                    else { // Includes air
                        if (rand.nextBoolean()) {
                            int randInt = this.random.nextInt(5);
                            if (randInt < 3) // 3/5 chance
                                this.world.setBlockState(pos.add(i, j, k), Blocks.ICE.getDefaultState());
                            else if (randInt == 3)// 1/5 chance
                                this.world.setBlockState(pos.add(i, j, k), Blocks.PACKED_ICE.getDefaultState());
                            else // 1/5 chance
                                this.world.setBlockState(pos.add(i, j, k), Blocks.BLUE_ICE.getDefaultState());
                        }
                    }
                }
            }
