package net.endplex.plexmod.entity.trident;

import net.endplex.plexmod.PlexModClient;
...
public class OceanTridentEntity extends TridentEntity {
    ...
    public OceanTridentEntity(EntityType<? extends OceanTridentEntity> entityType, World world) {
        super(entityType, world);
        this.tridentStack = new ItemStack(ModItems.OCEAN_TRIDENT);
    }

    public OceanTridentEntity(World world, LivingEntity owner, ItemStack stack) {
    ...
    public void tick() {
        ...
        if (i > 0 && (this.dealtDamage || this.isNoClip()) && entity != null) {
            if (!this.isOwnerAlive()) {
                ...
                // If touches lava, turn block into obsidian
                if (this.getBlockStateAtPos() == Blocks.LAVA.getDefaultState()) {
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        BlockPos pos = new BlockPos(entityHitResult.getPos());
        Random rand = new Random();
        EntityType.PUFFERFISH.spawn((ServerWorld) this.world, null, null, null, pos,
                SpawnReason.TRIGGERED, true, true);
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 0; j++) {
                for (int k = -1; k <= 1; k++) {
                    if (this.world.getBlockState(pos.add(i, j, k)) == Blocks.LAVA.getDefaultState()) {
                        int randInt = this.random.nextInt(2);
                        if (randInt < 1) // 2/3 chance
                            this.world.setBlockState(pos.add(i, j, k), Blocks.OBSIDIAN.getDefaultState());
                        else // 1/3 chance
                            this.world.setBlockState(pos.add(i, j, k), Blocks.STONE.getDefaultState());
                    }
                    else if (this.world.getBlockState(pos.add(i, j, k)) == Blocks.AIR.getDefaultState()) {
                        this.world.setBlockState(pos.add(i, j, k), Blocks.WATER.getDefaultState());
                    }
                }
            }
        }
    ...
    protected float getDragInWater() { return 1.5F; }
