...
public class EndTridentEntity extends TridentEntity {
    ...
    public EndTridentEntity(EntityType<? extends EndTridentEntity> entityType, World world) {
        super(entityType, world);
        this.tridentStack = new ItemStack(ModItems.END_TRIDENT);
    }
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        float g = 1.0F;
        // If it's night or user is in end dimension, and has channeling, summon enderman to fight
        if (this.world instanceof ServerWorld && (this.world.isNight() || this.world.getDimension().hasEnderDragonFight()) && this.hasChanneling()) {
            ...
            if (this.world.isSkyVisible(blockPos)) {
                ...
                enderman.setAttacker((LivingEntity) entity);
                enderman.setAngryAt(entity.getUuid());
                enderman.setTarget((LivingEntity) entity);
                enderman.setAngerTime(6000); // 5 minutes
                assert entity2 != null;
                enderman.setPosition(entity2.prevX, entity2.prevY, entity2.prevZ);
                this.world.spawnEntity(enderman);
                
                soundEvent = SoundEvents.ENTITY_ENDERMAN_SCREAM;
                g = 5.0F;
            }
        }
    ...
    protected void onCollision(HitResult hitResult) {
        // If user shifts, teleport user to trident's pos
        if (!this.world.isClient && Objects.requireNonNull(this.getOwner()).isSneaking()) {
            Vec3d pos = this.getPos();
            this.getOwner().requestTeleport(pos.x, pos.y, pos.z);
        }
